...
## Giới thiệu
Dự án này xây dựng chương trình điều khiển **robot Omni 3 bánh** sử dụng vi điều khiển **Arduino Mega**. Robot có khả năng di chuyển đa hướng trên mặt phẳng nhờ 3 bánh omni được bố trí lệch nhau 120 độ.
...
* Thiết kế bộ điều khiển PID vị trí cho các trục:
  * `x`
  * `y`
  * `theta`
...
* Hỗ trợ các chế độ di chuyển:
  * Đi thẳng theo quãng đường và góc đặt trước.
  * Đi tới tọa độ đích.
  * Dừng robot.
  * Reset tọa độ và encoder.
  * In trạng thái robot.
  * In giá trị encoder.
* Có phần code mở rộng cho quỹ đạo tròn được để trong comment.

## Công nghệ sử dụng
* Arduino IDE
* Ngôn ngữ lập trình C/C++ cho Arduino
...
Kết quả của PID vị trí là vận tốc đặt của robot:
```text
vxWorldRef
vyWorldRef
omegaRef
```
Các bộ PID vị trí gồm:
```cpp
pidX
pidY
pidTheta
...
### Chuyển đổi động học
Sau khi có vận tốc đặt trong hệ tọa độ toàn cục, chương trình chuyển đổi sang hệ tọa độ thân robot:
```cpp
worldToBody()
```
Sau đó sử dụng động học nghịch để tính tốc độ đặt cho từng bánh:
```cpp
inverseKinematic()
```
Kết quả thu được:
...
Các bộ PID tốc độ gồm:
```cpp
pidW[0]
pidW[1]
pidW[2]
```
Kết quả của PID tốc độ là tín hiệu PWM điều khiển 3 động cơ:
```text
pwmOut[0]
pwmOut[1]
pwmOut[2]
...
LINE 3.0 0
```
Robot sẽ di chuyển thẳng 3 mét theo trục X.
```text
GOTO 3.0 5.0
```
Robot sẽ di chuyển tới tọa độ `x = 3m`, `y = 5m`.
